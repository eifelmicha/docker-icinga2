...
create_api_user() {
  ...
  [[ -n "${ICINGA2_API_HACHECK_USER}" ]] &&  hacheck_user=$(echo ${ICINGA2_API_HACHECK_USER} | sed -e 's/,/ /g' -e 's/\s+/\n/g' | uniq)
...
then
  ...
  if [[ $(cat ${api_file} | wc -l) -gt 7 ]]
  then
    log_info "create configuration for HA-Check API user ..."

    # the initial configuration
    # make it blank and create our default users
    #

    for u in ${hacheck_user}
    do
      user=$(echo "${u}" | cut -d: -f1)
      pass=$(echo "${u}" | cut -d: -f2)
...
permissions = [
 {
   ...
   filter = {{ regex("^HA-Check", host.name) }}
